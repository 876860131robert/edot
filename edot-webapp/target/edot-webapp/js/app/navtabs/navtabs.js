...
(function($) {
  $.fn.navtabs = function() {
	...
	$(".J_menuItem").each(function (index) {
	    ...
	    //if($(this).attr('ui-sref') != '' && $(this).attr('href') == undefined){
	    //	$(this).attr('href', $(this).attr('ui-sref')).removeAttr('ui-sref');
	    //}
	});
...
function scrollToTab(element) {
    ...
    //可视区域tab宽度
    var visibleWidth = $(".content-tabs").parent("div").outerWidth(true) - tabOuterWidth;
    //实际滚动宽度
    var scrollVal = 0;
...
function menuItem() {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe onLoad=setIframeHeight("iframe'+dataIndex+'") class="J_iframe" name="iframe' + dataIndex + '" id="iframe' + dataIndex + '" width="100%" scrolling="no" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '"></iframe>';
...
function activeTab() {
	...
	/*if($(this).attr('href').indexOf('#') != -1){
		toggleTabs('1');// angualrjs方式
    }else{
    	toggleTabs('0');// iframe方式
    }*/
...
function menuItem2(url) {
	//toggleTabs('1');
	
	var $this = $(this);//$(".sidebar-menu li a[href*='"+url+"']");
	 var dataUrl = $this.attr('href'),
     dataIndex = $this.data('index'),
     menuName = $.trim($this.text()),
     flag = true;
	 ...
	 if (dataUrl == undefined || $.trim(dataUrl).length == 0)return false;
	    ...
	    // 选项卡菜单不存在
	    if (flag) {
	        var str = '<a href="'+ dataUrl +'" class="active J_menuTab" data-id="' + dataUrl + '">' + menuName + ' <i class="fa fa-times-circle"></i></a>';
	        $('.J_menuTab').removeClass('active');

	        // 添加选项卡对应的iframe
	        var uiSref = dataUrl.substr(2);
	        uiSref = uiSref.replace(/\//g,".");
	        var str1 = '<div class="app-content-body fade-in-up" ng-view="'+ uiSref +'"></div>';
	        $('.J_mainContent').find('iframe.J_iframe').hide().parents('.J_mainContent').append(str1);
...
function toggleTabs(val){
	if(val == '1'){
		$('.J_mainContent .J_iframe').each(function () {
			 $(this).siblings('.J_iframe').hide();
	    });
	}else{
		$('.J_mainContent .J_iframe').each(function () {
			 $(this).siblings('.J_iframe').show();
	    });
